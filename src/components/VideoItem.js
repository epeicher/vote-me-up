...
export default class VideoItem extends React.Component {
  getVoteIcon() {
    ...
    if (
      !userId ||
      !this.props.video.votedBy ||
      !this.props.video.votedBy[userId]
    ) {
      return (
        <StarBorder
  ...
  handleVote(e, id) {
    ...
    this.props.voteUp(id, this.props.match.params.list);
  }
  ...
  handleInput(e) {
    ...
    // enter
    if (e.which === 13)
      this.props.itemEdited(id, editedValue, this.props.match.params.list);
  }
  ...
  handleViewed(e, viewed, id) {
    ...
    this.props.viewed(id, viewed, this.props.match.params.list);
  }
    ...
    return (
      <Card>
        {!video.isEditing ? (
          ...
          >
            <ListItem
              primaryText={<ItemText txt={video.name} />}
              style={video.viewed ? styles.viewed : {}}
              key={video.id}
              secondaryText={
                <span>
                  <b>{video.votes}</b> votes
                </span>
              }
              ...
              rightIcon={
                <span style={styles.icons}>
                  ...
                  <ContentClear
                    color={red500}
                    onClick={e => this.handleDelete(e, video)}
                  />
                  <Toggle
                    style={styles.iconViewed}
                    toggled={video.viewed}
                    onToggle={(e, checked) =>
                      this.handleViewed(e, checked, video.id)
                    }
                  />
                </span>
              }
