...
export function getVotes(list) {
  return dispatch =>
    api.getListOfItems(items => dispatch(votedAction(items)), list);
}

export function voteUp(id, list) {
  return dispatch => {
    dispatch(voteUpAction(id));
    api.voteUp(id, list).then(() => dispatch(votedAction()));
  };
}

export function viewed(id, viewed, list) {
  return dispatch => {
    dispatch(viewedAction(id));
    api.viewed(id, viewed, list);
  };
}

export function addItem(name, list) {
  return dispatch =>
    api.addItem(addItemObject(name), list).then(dispatch({ type: ITEM_ADDED }));
}
...
export function itemEdited(id, value, list) {
  return dispatch => {
    dispatch({ type: ITEM_EDITING, id, isEditing: false });
    api.updateItem(id, value, list);
  };
}
...
export function deleteItem(id, list) {
  return dispatch =>
    api.deleteItem(id, list).then(() => dispatch({ type: ITEM_DELETED }));
}
